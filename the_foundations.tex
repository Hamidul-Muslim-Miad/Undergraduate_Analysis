...
\end{Definition}
\noindent Below is a diagram summarizing all the important relations we are going to encounter in this text :\vspace{0.5cm}
\resizebox{17.75cm}{6cm}{%
\begin{tikzpicture}[
	node distance=0.5cm and 0.01cm,
	box/.style={draw, rectangle, rounded corners, minimum width=1cm, minimum height=0.8cm, align=center},
	title/.style={font=\large\bfseries}
	]
	
	% Top level - Relation
	\node[box] (relation) {Relation};
	
	% First level branches
	\node[box, below left=of relation] (preorder) {Preordered Set\\ (Reflexive + Transitive)};
	\node[box, below right=of relation] (function) {Function\\ (Right-unique + Left-total)};
	
	% Connect top to first level
	\draw[->] (relation) -- (preorder);
	\draw[->] (relation) -- (function);
	
	% Preorder branch
	\node[box, below left=of preorder] (equivalence) {Equivalence Relation\\ (+ Symmetric)};
	\node[box, below =of preorder] (partial) {Partial Order\\ (+ Antisymmetric)};
	
	% Connect preorder to its children
	\draw[->] (preorder) -- (equivalence);
	\draw[->] (preorder) -- (partial);
	
	% Partial order to total order
	\node[box, below=of partial] (total) {Total Order\\ (+ Connectedness)};
	\draw[->] (partial) -- (total);
	
	% Function branch
	\node[box, below =of function] (injective) {Injective Function\\ (+ Left-unique)};
	\node[box, below right=of function] (surjective) {Surjective Function\\ (+ Right-total)};
	
	% Connect function to its children
	\draw[->] (function) -- (injective);
	\draw[->] (function) -- (surjective);
	
	% Bijective function at intersection
	\node[box, below=3cm of function] (bijective) {Bijective Function\\ (Injective + Surjective)};
	
	% Connect to bijective
	\draw[->] (injective) -- (bijective);
	\draw[->] (surjective) -- (bijective);
...
\end{Definition}
\subsection{More about Metric Spaces}
